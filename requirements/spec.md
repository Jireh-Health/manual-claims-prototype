...
**Experience:** An admin opens the Wallet tab and sees 500,000 KES available across 50 settled invoices. Instead of disbursing everything, they use the checkboxes to select 10 specific high-value invoices totaling 250,000 KES. They click "Disburse." A confirmation step appears showing the selected invoice count, total amount, and a system-generated code (e.g., `DISB-7X4K`). The admin types the code exactly and clicks "Confirm Disburse." The invoices move to "Disbursing." When M-Pesa confirms the transfer, a green success alert appears: *"Disbursement successful — KES 250,000 sent via M-Pesa. Ref: QRT9ABC12."* The alert auto-dismisses after 5 seconds. The invoices move to the History tab and the wallet balance updates.
...
**Experience:** An admin initiates a 100,000 KES disbursement. The M-Pesa Paybill request times out and the status flips to "Disbursement Failed." A persistent red error banner appears in the Wallet: *"Payout could not be completed. Please try again."* with a Retry button inline. The invoices are immediately unlocked so no funds are stuck. The admin clicks "Retry" directly from the banner or from the failed invoice row. The system resubmits through the same preconfigured M-Pesa Paybill. On success, the red error banner is dismissed and a green success alert replaces it: *"Disbursement successful — KES 100,000 sent via M-Pesa. Ref: QRT4XYZ99."* The invoices move to the History tab.
...
An admin opens the Wallet tab and selects 8 settled invoices totaling 180,000 KES. They click "Disburse." A confirmation step appears displaying a summary of the payout (invoice count, total amount) alongside a system-generated code (e.g., `DISB-9M2R`). The admin types the code exactly — the "Confirm Disburse" button remains disabled until the code matches. Once confirmed, the selected invoices immediately move to a "Disbursing" state and become non-interactive. Within moments, the system receives a success callback from M-Pesa. A green success alert banner appears at the top of the Wallet: *"Disbursement successful — KES 180,000 sent via M-Pesa. Ref: QRT7MNP33."* The alert auto-dismisses after 5 seconds. The invoices transition to "Disbursed," disappear from the active Wallet list, appear in the History tab with the full payout metadata, and the wallet balance updates.

**Error outcomes:**

The following failure cases can occur after a disbursement is initiated, each with a specific alert:

- **Network/timeout failure:** M-Pesa does not respond within the expected window. The invoices return to the Wallet with "Disbursement Failed" badges. A persistent red error banner appears: *"Payout could not be completed. Please try again."* with an inline Retry button. The banner stays visible until the admin retries or manually dismisses it. No funds are deducted.
- **Invalid Paybill or account number:** The preconfigured Paybill or account number is rejected by M-Pesa. The invoices are unlocked in the Wallet. A persistent red error banner appears: *"Paybill validation failed. Please contact support to update the disbursement configuration."* No Retry button is shown since retrying with the same configuration would fail again.
- **Partial failure (bulk payout):** When disbursing a large batch, M-Pesa may confirm some transfers while rejecting others. A yellow warning banner appears: *"Partial disbursement — X of Y invoices were paid out successfully. Z invoices failed and have been returned to your Wallet."* Successfully transferred invoices move to "Disbursed" in the History tab. Failed ones return with individual "Disbursement Failed" badges for independent retry.
...
| **Disbursement Status Feedback** | As an Admin, I want real-time status updates during disbursement, so that I know whether the payout succeeded or failed. | 1. Selected invoices immediately show "Disbursing" status and become non-interactive once the confirmation code is accepted. 2. Successful payouts transition invoices to "Disbursed" and move them to the History tab with the M-Pesa reference number. 3. A green success alert banner — e.g. *"Disbursement successful — KES 250,000 sent via M-Pesa. Ref: QRT9ABC12."* — appears at the top of the Wallet and auto-dismisses after 5 seconds. 4. Failed payouts return invoices to the Wallet with a "Disbursement Failed" badge and a persistent red error banner — e.g. *"Payout could not be completed. Please try again."* — with an inline Retry button that remains visible until dismissed or retried. 5. On a successful retry, the red error banner is replaced by the green success alert. |
| **Disbursement Retry** | As an Admin, I want to retry a failed disbursement without re-selecting invoices, so that I can recover from transient errors quickly. | 1. "Retry" action is available on invoices marked "Disbursement Failed." 2. Retry resubmits through the same preconfigured M-Pesa Paybill. 3. Invoices remain in the Wallet and are fully interactable while in the failed state. 4. On retry success, the persistent red error banner is cleared and a green success alert appears, auto-dismissing after 5 seconds. 5. On retry failure, the red error banner remains with the Retry button still active. |
| **Partial Batch Disbursement Failure** | As an Admin, I want individual invoice failures within a bulk disbursement to be isolated, so that successfully paid invoices are not affected. | 1. Invoices that transfer successfully move to "Disbursed" in the History tab. 2. Invoices that fail return individually to the Wallet with "Disbursement Failed" badges. 3. The admin can retry each failed invoice independently. 4. A yellow partial-failure warning banner is displayed — e.g. *"Partial disbursement: 8 of 10 invoices paid. 2 invoices failed and remain in your Wallet."* — and persists until dismissed. 5. No green success banner is shown when any invoices have failed; the yellow warning takes precedence. |
